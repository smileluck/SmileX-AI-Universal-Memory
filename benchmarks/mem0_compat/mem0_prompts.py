"""mem0 官方评测 prompt 移植(逐字,英文原文).

来源: https://github.com/mem0ai/memory-benchmarks
  benchmarks/locomo/prompts.py     (2026-08 抓取)
  benchmarks/longmemeval/prompts.py

口径要点(与自家 benchmarks/locomo|longmemeval 的严口径相反):
- LoCoMo 作答: 禁止拒答("NEVER say not specified"),7 步推理模板
- LoCoMo 判卷: 宽松 judge(PARTIAL CREDIT / PARAPHRASES / 日期±14天),
  输出 JSON {reasoning, label: CORRECT|WRONG}
- LongMemEval 作答: 14 条规则 + <mem_thinking>,拒答话术
  "The information provided is not enough"
- LongMemEval 判卷: anti-"no" 偏差校正,输出 yes/no

注意: LoCoMo category 映射以 mem0 实现为准(与论文编号不同!):
  1=multi-hop(282) 2=temporal(321) 3=open-domain(96) 4=single-hop(841)
  5=adversarial(446, 不计分)
"""
...
LOCOMO_CATEGORIES_TO_EVALUATE = [1, 2, 3, 4]

LOCOMO_ANSWER_PROMPT = """You are answering a question using retrieved memories from past conversations. Follow these reasoning steps IN ORDER.
...
LOCOMO_ANSWERER_MEMORY_LIMIT = 200
...
LOCOMO_JUDGE_PROMPT = """Label the generated answer as CORRECT or WRONG.
...
def locomo_preprocess_answer(category: int, answer: str) -> str:
    """mem0 同款: category 3(open-domain)的 gold 取分号前第一段."""
    if category == 3 and ";" in answer:
        return answer.split(";")[0].strip()
    return answer


def parse_locomo_judge(text: str) -> tuple[bool, str]:
    """解析 mem0 LoCoMo judge 输出(JSON label CORRECT/WRONG)."""
    try:
        data = json.loads(text)
        label = str(data.get("label", "")).upper()
        return ("CORRECT" in label and "WRONG" not in label), label
    except (json.JSONDecodeError, AttributeError):
        m = re.search(r"\b(CORRECT|WRONG)\b", text.upper())
        if m is None:
            return False, text.strip()[:80]
        return m.group(1) == "CORRECT", m.group(1)
...
LME_ANSWER_PROMPT = """You are a personal assistant with access to memories from past conversations with a user. Answer the question using information from the memories below. Be direct and concise.
...
def parse_lme_judge(text: str) -> tuple[bool, str]:
    """解析 mem0 LongMemEval judge 输出(yes/no, 取最后一个)."""
...
def strip_mem_thinking(answer: str) -> str:
    """去掉 <mem_thinking> 隐藏推理,只留可见答案(mem0 同款)."""
    return re.sub(r"<mem_thinking>.*?</mem_thinking>", "", answer, flags=re.DOTALL).strip()
