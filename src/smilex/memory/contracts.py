...
- WriteStatus / BootstrapStage 为 API 契约级 StrEnum(不入 Layer 0 enums.py)
- 所有 DTO 提供 to_dict/from_dict 往返序列化

位置说明: 原 middlewares/dto.py 下沉至此(2026-09)。DTO 只依赖 models+utils,
同时服务 middleware/server/bootstrap 三层契约,置于 memory 包根可消除
scheduler.bootstrap → middlewares 的反向依赖;旧路径经 shim 保留兼容。
"""
