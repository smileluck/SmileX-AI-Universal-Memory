...
| P1 社区检测 | **已升级落地(2026-09-09)**: semantic 任务从连通分量升级为 Louvain 轻量版(详见右列) | networkx 内置 `louvain_communities`(未引入 igraph/leidenalg)+ 固定 seed + 实体/边加载 ORDER BY(确定性前提)+ 加权无向图(实体对间三元组条数为边权,加权模块度切巨分量弱桥)+ **分宇宙建图**(global 单独 + 每个 project/tenant ∪ global,对齐 recall 默认检索视角,修复旧版全库混图的跨项目污染)+ **diff 增量刷新**(社区 key 与库内 L3 缓存 diff,数据未变重跑零写入;消失社区先清向量再删行,FK 安全)+ 注入 vector_store 后社区缓存即时重嵌进 KNN 通道(修复 L3 不进 KNN)。**LLM 摘要升级有意不做** — 非确定性内容会让 diff 永远判定"有变化",破坏零写入幂等;懒生成设计一并否决(检索路径无触发钩子,读时写库违反既有原则)。EntityCommunity 表不建(无按成员 JOIN 的消费方,成员 id 已存 fragment 的 entities 列);"L3.5"命名作废,统一 L3 |
