...
**2026-09-09 二轮(子系统深审 → 接线修复批)已落地**: L0 持久化闭环
(close_session 全量晋升 + memory_session_end 工具 + 事件接线)、调度独立
维护引擎 + archiver/promoter 默认注册 + maintain CLI、矛盾检测写入留痕、
抽取降级可观测、consolidate 锚点继承、指纹过期清理、TTL 链路、/api/errors
+ 面板聚焦字段与错误指纹 tab、causal 任务/表删除(迁移 016)、hybrid 移除
恒空 spatial、as-built 诚实化(§9 两道防线/§10 接线状态/§11b 已知限制)。

**剩余可选项**(按价值排序,均为"按需再做"而非待办):
1. 个人成长模块 Phase 1(10 号 v2 — 0.5 天约定层: MCP instructions 写入约定 + skill_level 三元组约定)
2. 快照含归档表开关 + 导出的 CLI/MCP 用户入口(11 号修订记录)
3. ConflictDetector 乐观版本接入或删除(§9 预留状态,待真实 lost-update 需求)
4. 矛盾 scan 巡检调度化/矛盾报告落表(§10,需要时接)
5. PG 适配器/分布式锁等(02/05 §5 — 被多用户需求阻塞,远期)
